...
class OLPLS(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        if self.n == 0:
            ...
            self.C = np.zeros((n_labels, self.n_components))
            ...
            for c in range(0, self.n_components):
                ...
                wtemp = wtemp + self.mu * (np.dot(np.dot(np.dot(np.dot(-wtemp.T, Suse), Suse.T), wtemp), wtemp)) / np.dot(wtemp.T, wtemp)**2  \
                    + np.dot(np.dot(Suse, Suse.T), wtemp)/np.dot(wtemp.T, wtemp)
                wtemp = wtemp / np.linalg.norm(wtemp)
                self.W[:, c] = wtemp
